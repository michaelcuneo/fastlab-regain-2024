<script lang="ts">
  ...
  import Button, { Label as ButtonLabel } from "@smui/button";
	...
	import { userGroups, activeAreas, user } from '$lib/utils/store';
	...
	$effect(() => {
		if (userGroups?.current.length >= 0) {
	...
	/*
	// Save Active Areas for User as User Groups
	// No need to redownload the user, because it will be injected client side as settings updates.
	// Perform a load of the exercises for selected User Groups.
	*/
</script>
	...
	<Cell span={8}>
		<Card>
			<CardContent>
				...
				<h4>OPT OUT</h4>
				<div>
					<p>If you feel like you do not want to be a part of this research anymore, you can click this button, and we will permanently delete all of your data. This will permanently delete all record of your participation with Regain, and log you out.</p>
					<Button class="error-button" variant="raised" onclick={() => open = !open}>PERMANENTLY REMOVE ALL OF MY DATA</Button>
				</div>
			</CardContent>	
		</Card>
	</Cell>
